...
if __name__ == '__main__':
    ...
    resnet_kwargs={'init_num_filters':64, 'block_sizes': [3, 4, 6, 3], 'block_strides': [1, 2, 2, 2],}


    net = unet.Ugan(cost="cross_entropy", channels=3, n_class=2, border_addition=2, patch_size=1000, summaries=True, unet_kwargs=unet_kwargs, resnet_kwargs=resnet_kwargs)
    ...
    data_provider = \
        image_util.ImageDataProvider("Potsdam/resized2/train/*.tif", "Potsdam/bin_labels_resized/", patch_size=1000,
                                                 channels=3, n_class=2, border_size=net.offset//2+2, data_suffix="_RGB.tif",
    ...
    trainer = unet.Trainer(net, batch_size=1, optimizer='adam', g_opt_kwargs={'learning_rate':2e-4, 'beta1':0.5})
